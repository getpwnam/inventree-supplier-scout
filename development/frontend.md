# Supplier Scout- Frontend Code

This directory contains the frontend code for the Supplier Scout plugin.
...
This will compile the frontend into the `../supplier_scout/static` directory (ready for distribution).
...
For local branch-switch testing with InvenTree dev static serving, run from repository root:

```bash
./development/scripts/refresh-branch-static.sh
```

This rebuilds frontend assets and syncs `supplier_scout/static/` into
`/home/inventree/dev/static/plugins/supplierscout/`.

Developer workflow notes are in `./README.md`.
